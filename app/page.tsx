    ...
    <div className="bg-white min-h-screen text-slate-900 flex flex-col font-inter overflow-hidden selection:bg-emerald-100 selection:text-emerald-900">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-8">
             {[
               ...
               { name: "7-Day Pro Trial", price: "100", period: "/7 days", badge: "🔥 Best First Step", features: ["AI Risk Guard Engine", "Unlimited Portfolio Logging", "Tax-Loss Harvesting AI", "NSE Charts + MACD/RSI", "Priority Support"], cta: "Start Trial — KES 100", popular: true },
               { name: "Institutional", price: "Custom", period: "", badge: "Family Offices", features: ["Custom Portfolio APIs", "Dedicated Yield Analyst", "Audit-Grade Reporting", "Concierge Onboarding"], cta: "Contact Sales", popular: false },
             ].map((pkg, i) => (
               <div key={i} className={`relative p-10 rounded-[2.5rem] border ${pkg.popular ? "border-emerald-600 bg-slate-900 text-white shadow-2xl" : "border-slate-200 bg-white text-slate-900 shadow-sm"} flex flex-col items-center text-center overflow-hidden h-full group`}>
                  ...
                  <h4 className="text-[11px] font-black uppercase tracking-[0.2em] mb-6 opacity-60">{pkg.name}</h4>
                  <div className="mb-8">
                    <span className="text-4xl font-black tracking-tighter">KES {pkg.price}</span>
                    {pkg.price !== "Free" && pkg.price !== "Custom" && <span className="text-[10px] font-bold uppercase ml-2 opacity-40">/ month</span>}
                  </div>
                  ...
                  <button 
                    onClick={() => {
                      ...
                      } else if (pkg.name === "7-Day Pro Trial") {
                        if (isLoggedIn) {
                          setPremiumModalOpen(true);
                        } else {
                          router.push("/packages");
                  ...
                  >
                    {isLoggedIn && pkg.name === "S-Tier Pro" && user?.isPremium ? "Active Subscription" : pkg.cta}
                  </button>
               </div>
             ))}
          </div>
      ...
      <section className="relative h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-6 relative z-10 text-center space-y-8">
           <div className="inline-flex items-center gap-2 px-4 py-2 bg-rose-50 border border-rose-200 rounded-full text-[9px] font-black text-rose-600 uppercase tracking-widest">
              <ShieldCheck className="w-3.5 h-3.5" /> Institutional Safety Shield
           </div>
           ...
           <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em] max-w-2xl mx-auto leading-relaxed">
              SENTILL IS A NON-CUSTODIAL DATA TERMINAL. ALL INVESTMENTS SHOULD BE CONDUCTED THROUGH LICENSED ENTITIES AFTER INDEPENDENT VERIFICATION.
           </p>
        </div>
      </section>
