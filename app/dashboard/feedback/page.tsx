...
import { MessageSquare, Send, Zap, Star, ShieldCheck, Heart } from "lucide-react";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast.success("Feedback Transmitted", {
        description: "Your insights have been prioritized by the Neural Core."
      });
    }, 1500);
  };
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-8">
           <div className="space-y-2">
              ...
              <select className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-black text-slate-900 uppercase tracking-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all appearance-none cursor-pointer">
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-3">
                 ...
                 <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map(s => (
                      <button key={s} type="button" className="text-amber-400 hover:scale-110 transition-transform"><Star className="w-5 h-5" /></button>
                    ))}
                 </div>
              </div>
